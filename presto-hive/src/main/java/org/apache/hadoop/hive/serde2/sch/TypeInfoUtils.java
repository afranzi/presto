 ...
 */
package org.apache.hadoop.hive.serde2.sch;
...
public final class TypeInfoUtils
{
     ...
     *
     * @param size In case the last parameter of Method is an array, we will try to
    ...
    private static class TypeInfoParser
    {
        ...
        private static boolean isQuoted(char c)
        {
            return c == '"';
        }
        ...
        private static ArrayList<Token> tokenize(String typeInfoString)
        {
            ...
            while (end <= typeInfoString.length()) {
                ...
                if (isQuoted(typeInfoString.charAt(end - 1))) {
                    isQuoted = !isQuoted;
                }
                if (end == typeInfoString.length()
                        || (
                        (!isTypeChar(typeInfoString.charAt(end - 1)) || !isTypeChar(typeInfoString.charAt(end)))
                                && !isQuoted
                )) {
                    Token t = new Token();
                    t.position = begin;
                    t.text = typeInfoString.substring(begin, end).replaceAll("\"", "");
                    t.isType = isTypeChar(typeInfoString.charAt(begin)) || isQuoted(typeInfoString.charAt(begin));
                    tokens.add(t);
                    begin = end;
                }
                end++;
